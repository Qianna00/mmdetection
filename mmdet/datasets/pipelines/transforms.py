...
@PIPELINES.register_module()
class ResizeImgPair(Resize):
    def __init__(self, img_scale=None,
                 multiscale_mode='range',
                 ratio_range=None,
                 keep_ratio=True):
        super(ResizeImgPair, self).__init__(img_scale=img_scale,
    ...
    def _resize_img(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            results['scale_factor'] = scale_factor
            results['keep_ratio'] = self.keep_ratio
        for key in results.get('img_lr_fields', ['img_lr']):
            w = int(results['scale'][0] / 2)
            h = int(results['scale'][1] / 2)
            scale_lr = (w, h)
            if self.keep_ratio:
                img_lr, scale_factor = mmcv.imrescale(
                    results[key], scale_lr, return_scale=True)
            else:
                img_lr, w_scale, h_scale = mmcv.imresize(
                    results[key], scale_lr, return_scale=True)
            results[key] = img_lr
...
class PairPad(Pad):
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_lr_fields', ['img_lr']):
            if self.size is not None:
                padded_img_lr = mmcv.impad(results[key], self.size, self.pad_val)
            elif self.size_divisor is not None:
                padded_img_lr = mmcv.impad_to_multiple(
                    results[key], self.size_divisor, pad_val=self.pad_val)
            results[key] = padded_img_lr
        results['pad_shape'] = padded_img.shape
        results['pad_fixed_size'] = self.size
        results['pad_size_divisor'] = self.size_divisor
