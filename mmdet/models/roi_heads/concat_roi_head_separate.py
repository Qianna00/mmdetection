...
class ConcatRoIHeadSeparate(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    """Simplest base roi head including one bbox head and one mask head.
    """
    def __init__(self, **kwargs):
        super(ConcatRoIHeadSeparate, self).__init__(**kwargs)
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            bbox_results = self._bbox_forward_train(x, x_extra, sampling_results,
                                                    gt_bboxes, gt_labels,
                                                    img_metas)
    ...
    def _bbox_forward(self, x, x_extra, rois):
        # TODO: a more flexible way to decide which feature maps to use
        bbox_feats = self.bbox_roi_extractor(
            x[:self.bbox_roi_extractor.num_inputs], rois)
        bbox_feats_extra = self.bbox_roi_extractor(
            x_extra[:self.bbox_roi_extractor.num_inputs], rois, feat_detach=True)
        bbox_feats_concat = self.conv_cat(torch.cat([bbox_feats, bbox_feats_extra], dim=1))
        # if self.with_shared_head:
            # bbox_feats = self.shared_head(bbox_feats)
        cls_score, bbox_pred = self.bbox_head(bbox_feats_concat, bbox_feats)
