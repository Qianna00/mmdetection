...
class RoIHeadGan(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward(self, x, rois, x_lr=None):
        ...
        if self.with_fsr_generator:
            ...
            bbox_feats_sr = self.fsr_generator((bbox_feats_sub_hr, bbox_feats_hr))
            if x_lr is not None:
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        one_weights = torch.Tensor(np.ones(rois.shape[0])).cuda()
        zero_weights = torch.Tensor(np.zeros(rois.shape[0])).cuda()
        zero_weights_4 = torch.Tensor(np.zeros((rois.shape[0], 4))).cuda()
        loss_weights_adv_real = torch.where(areas > 96*96, one_weights, zero_weights)
        loss_weights_adv_fake = torch.where(areas <= 96*96*4, one_weights, zero_weights)
        ...
        loss_g_dis = self.dis_head.loss(bbox_results['gen_score_sr'], target_ones_g, loss_weights_adv_fake)
        loss_d_sr = self.dis_head.loss(bbox_results['dis_score_sr'], target_zeros_d, loss_weights_adv_fake)
        loss_d_hr = self.dis_head.loss(bbox_results['dis_score_hr'], target_ones_d, loss_weights_adv_real)
