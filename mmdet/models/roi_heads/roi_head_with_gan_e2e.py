...
class RoIHeadGane2e(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(RoIHeadGane2e, self).__init__()
    ...
    def forward_train(self,
        ...
        if self.with_bbox or self.with_mask:
            ...
            for i in range(num_imgs):
                ...
                sampling_result = self.bbox_sampler.sample(
                    ...
                    gt_bboxes[i],
                    gt_labels[i],
                    feats=[lvl_feat[i][None] for lvl_feat in tuple([x[1]])])
        ...
        if self.with_bbox:
            ...
            losses.update(loss_cls_lr=bbox_results['loss_bbox_lr']['loss_cls'])
            losses.update(loss_bbox_lr=bbox_results['loss_bbox_lr']['loss_bbox'])
            losses.update(acc_lr=bbox_results['loss_bbox_lr']['acc'])
            """if self.with_dis_head:
    ...
    def _bbox_forward(self, x, rois, x_lr=None):
        ...
        if self.with_dis_head:
            dis_score_hr = self.dis_head(bbox_feats_hr)
            ...
            if x_lr is not None:
                ...
                gen_score_sr = self.dis_head(bbox_feats_sr)
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        target_ones_g = torch.Tensor(np.ones(rois.shape[0], 1)).cuda().long()
        target_ones_d = torch.Tensor(np.ones(rois.shape[0], 1)).cuda().long()
        target_zeros_d = torch.Tensor(np.zeros(rois.shape[0], 1)).cuda().long()
        ...
        loss_det = loss_bbox['loss_cls'] + loss_bbox['loss_bbox'] + loss_bbox_lr['loss_cls'] + loss_bbox_lr['loss_bbox']
        loss_gen = loss_bbox_lr['loss_cls'] + loss_bbox_lr['loss_bbox'] + loss_g_dis
        loss_dis = (self.dis_head.loss(bbox_results['dis_score_sr'], target_zeros_d) +
                    self.dis_head.loss(bbox_results['dis_score_hr'], target_ones_d)) / 2
