...
def train_detector_e2e(model,
    ...
    for key, value in dict(model.roi_head.bbox_head.named_parameters()).items():
        if value.requires_grad:
            params_b += [{'params': [value]}]
    optimizer_b = torch.optim.SGD(params_b, lr=cfg.optimizer_b.lr, momentum=cfg.optimizer_b.momentum,
                                  weight_decay=cfg.optimizer_b.weight_decay)
    optimizer_g = None
    optimizer_d = None
